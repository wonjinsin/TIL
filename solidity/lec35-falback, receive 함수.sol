...
또한 컨트랙트가 순수하게 Ether를 전송받는 경우도 fallback함수가 실행된다.
컨트랙트에 fallback함수가 없으면, 오류가 발생하고 ether는 반송되므로 컨트랙트가 함수를 거치지 않고 Ether를 받고 싶으면 반드시 fallback함수를 구현해야한다.
...
contract You {
    ...
    function DepositWithSend(address payable _to) public payable {
        bool success = _to.send(msg.value); // 이거 부르면 위에 fallback함수가 불림, 존재하지 않기 때문에
        require(success, "Failled");
    }

    function DepositWithTransfer(address payable _to) public payable {// 이거랑 아래도 마찬가지, 돈 보내느거면 fallback 함수가 항상 불림
    ...
    function JustGiveMessageWithFunds(address payable _to) public payable {
        // ~ 0.7
        (bool sent, ) = _to.call.value(msg.value)("HI");  // hi라는 method가 없어서 fallback 불림
        require(sent, "Failed to send either");
